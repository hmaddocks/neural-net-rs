...
begin
  File.open(filename, "rb") do |file|
    ...
    magic_number = header[0..3].pack('C*').unpack1('N')
    num_images = header[4..7].pack('C*').unpack1('N')
    num_rows = header[8..11].pack('C*').unpack1('N')
    num_cols = header[12..15].pack('C*').unpack1('N')
